...
import uk.co.real_logic.artio.builder.MassQuoteEncoder;
...
import static uk.co.real_logic.artio.builder.MassQuoteEncoder.QuoteSetsGroupEncoder;
...
import static uk.co.real_logic.artio.builder.MassQuoteEncoder.QuoteSetsGroupEncoder.QuoteEntriesGroupEncoder;
...
import static uk.co.real_logic.artio.builder.MassQuoteEncoder.QuoteSetsGroupEncoder.QuoteEntriesGroupEncoder.LegsGroupEncoder;
...
public class MassQuoteDecoder extends CommonDecoderImpl implements PartiesDecoder, MessageDecoder
{
...
public class QuoteSetsGroupDecoder extends CommonDecoderImpl implements UnderlyingInstrumentDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            REQUIRED_FIELDS.add(Constants.QUOTE_SET_ID);
            REQUIRED_FIELDS.add(Constants.TOT_NO_QUOTE_ENTRIES);
            REQUIRED_FIELDS.add(Constants.NO_QUOTE_ENTRIES_GROUP_COUNTER);
        }
    }
    ...
    {
        return new String(quoteSetID, 0, quoteSetIDLength);
    }
 ...
 return UnderlyingPutOrCall.NULL_VAL;
    ...
    /* QuoteSetValidUntilTime = 367 */
    public AsciiSequenceView quoteSetValidUntilTime(final AsciiSequenceView view)
    {
        if (!hasQuoteSetValidUntilTime)
        {
            throw new IllegalArgumentException("No value for optional field: QuoteSetValidUntilTime");
        ...
        return view.wrap(buffer, quoteSetValidUntilTimeOffset, quoteSetValidUntilTimeLength);
    }
...
public class QuoteEntriesGroupDecoder extends CommonDecoderImpl implements InstrumentDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            REQUIRED_FIELDS.add(Constants.QUOTE_ENTRY_ID);
        }
    }

    public final IntHashSet GROUP_FIELDS = new IntHashSet(130);
    ...
    private IntHashSet seenFields = new IntHashSet(52);
    ...
    {
        return new String(quoteEntryID, 0, quoteEntryIDLength);
    }
...
public class LegsGroupDecoder extends CommonDecoderImpl implements InstrumentLegDecoder
{
 ...
 return OrdType.NULL_VAL;
    ...
    public MassQuoteEncoder.QuoteSetsGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((MassQuoteEncoder.QuoteSetsGroupEncoder)encoder);
    }

    public MassQuoteEncoder.QuoteSetsGroupEncoder toEncoder(final MassQuoteEncoder.QuoteSetsGroupEncoder encoder)
    {
        encoder.reset();
    ...
    {
        private final MassQuoteDecoder parent;
        private int remainder;
        private QuoteSetsGroupDecoder current;

        public QuoteSetsGroupIterator(final MassQuoteDecoder parent)
        {
            this.parent = parent;
        }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.DEF_BID_SIZE:
                hasDefBidSize = true;
                defBidSize = getFloat(buffer, defBidSize, valueOffset, valueLength, 293, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.DEF_OFFER_SIZE:
                hasDefOfferSize = true;
                defOfferSize = getFloat(buffer, defOfferSize, valueOffset, valueLength, 294, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;
