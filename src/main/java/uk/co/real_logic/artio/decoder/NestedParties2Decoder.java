...
public class Nested2PartyIDsGroupDecoder extends CommonDecoderImpl
{
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_VALIDATION_ENABLED && nested2PartyIDSourceLength > 1)
        {
            invalidTagId = 758;
            rejectReason = 5;
            return false;
    ...
    private int nested2PartyIDSourceLength = 0;
    public int nested2PartyIDSourceLength()    {
       return nested2PartyIDSourceLength;
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.NESTED2_PARTY_ID_SOURCE:
                ...
                nested2PartyIDSourceLength = valueLength;
                break;
            ...
            case Constants.NO_NESTED2_PARTY_SUB_IDS_GROUP_COUNTER:
                ...
                for (int i = 0; i < noNested2PartySubIDsGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = nested2PartySubIDsGroupCurrent.decode(buffer, position, end - position);
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (nested2PartySubIDsGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (nested2PartySubIDsGroupCurrent != null) 
                            {
