...
public class SettlInstGroupDecoder extends CommonDecoderImpl implements PartiesDecoder, SettlInstructionsDataDecoder
{
    ...
    public boolean validate()
    {
        ...
        if (hasSettlInstTransType)
        {
        if (CODEC_VALIDATION_ENABLED && settlInstTransTypeLength > 1)
        {
            invalidTagId = 163;
            rejectReason = 5;
            return false;
    ...
    private int settlInstTransTypeLength = 0;
    public int settlInstTransTypeLength()    {
       return settlInstTransTypeLength;
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.SETTL_INST_TRANS_TYPE:
                ...
                settlInstTransTypeLength = valueLength;
                break;
        ...
        while (position < end)
        {
            ...
            case Constants.NO_SETTL_INST_GROUP_COUNTER:
                ...
                for (int i = 0; i < noSettlInstGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = settlInstGroupCurrent.decode(buffer, position, end - position);
                        if (positionIter == 0 && CODEC_VALIDATION_ENABLED)
                        {
                                invalidTagId = tag;
                                rejectReason = 16;
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (settlInstGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (settlInstGroupCurrent != null) 
                            {
