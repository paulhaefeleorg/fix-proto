...
public class Nested3PartyIDsGroupDecoder extends CommonDecoderImpl
{
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_VALIDATION_ENABLED && nested3PartyIDSourceLength > 1)
        {
            invalidTagId = 950;
            rejectReason = 5;
            return false;
    ...
    /* Nested3PartyID = 949 */
    public AsciiSequenceView nested3PartyID(final AsciiSequenceView view)
    {
        if (!hasNested3PartyID)
        {
            throw new IllegalArgumentException("No value for optional field: Nested3PartyID");
        ...
        return view.wrap(buffer, nested3PartyIDOffset, nested3PartyIDLength);
    }
    ...
    private int nested3PartyIDSourceLength = 0;
    public int nested3PartyIDSourceLength()    {
       return nested3PartyIDSourceLength;
    }
...
public class Nested3PartySubIDsGroupDecoder extends CommonDecoderImpl
{
    ...
    /* Nested3PartySubID = 953 */
    public AsciiSequenceView nested3PartySubID(final AsciiSequenceView view)
    {
        if (!hasNested3PartySubID)
        {
            throw new IllegalArgumentException("No value for optional field: Nested3PartySubID");
        ...
        return view.wrap(buffer, nested3PartySubIDOffset, nested3PartySubIDLength);
    }


    private final CharArrayWrapper nested3PartySubIDWrapper = new CharArrayWrapper();
    ...
    public Nested3PartyIDsGroupEncoder.Nested3PartySubIDsGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((Nested3PartyIDsGroupEncoder.Nested3PartySubIDsGroupEncoder)encoder);
    }

    public Nested3PartyIDsGroupEncoder.Nested3PartySubIDsGroupEncoder toEncoder(final Nested3PartyIDsGroupEncoder.Nested3PartySubIDsGroupEncoder encoder)
    {
        encoder.reset();
        if (hasNested3PartySubID())
        {
    ...
    /* NoNested3PartySubIDsGroupCounter = 952 */
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.NESTED3_PARTY_ID_SOURCE:
                ...
                nested3PartyIDSourceLength = valueLength;
                break;
            ...
            case Constants.NO_NESTED3_PARTY_SUB_IDS_GROUP_COUNTER:
                ...
                for (int i = 0; i < noNested3PartySubIDsGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = nested3PartySubIDsGroupCurrent.decode(buffer, position, end - position);
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (nested3PartySubIDsGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (nested3PartySubIDsGroupCurrent != null) 
                            {
