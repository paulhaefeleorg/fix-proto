...
public class PositionsGroupDecoder extends CommonDecoderImpl implements NestedPartiesDecoder
{
    ...
    /* PosType = 703 */
    public AsciiSequenceView posType(final AsciiSequenceView view)
    {
        if (!hasPosType)
        {
            throw new IllegalArgumentException("No value for optional field: PosType");
        ...
        return view.wrap(buffer, posTypeOffset, posTypeLength);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.LONG_QTY:
                hasLongQty = true;
                longQty = getFloat(buffer, longQty, valueOffset, valueLength, 704, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.SHORT_QTY:
                hasShortQty = true;
                shortQty = getFloat(buffer, shortQty, valueOffset, valueLength, 705, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;
