...
import uk.co.real_logic.artio.builder.MarketDataRequestRejectEncoder;
import static uk.co.real_logic.artio.builder.MarketDataRequestRejectEncoder.AltMDSourceGroupEncoder;
...
public class MarketDataRequestRejectDecoder extends CommonDecoderImpl implements MessageDecoder
{
    ...
    public boolean validate()
    {
        ...
        if (hasMDReqRejReason)
        {
        if (CODEC_VALIDATION_ENABLED && mDReqRejReasonLength > 1)
        {
            invalidTagId = 281;
            rejectReason = 5;
            return false;
    ...
    private int mDReqRejReasonLength = 0;
    public int mDReqRejReasonLength()    {
       return mDReqRejReasonLength;
    }
...
public class AltMDSourceGroupDecoder extends CommonDecoderImpl
{
    ...
    /* AltMDSourceID = 817 */
    public AsciiSequenceView altMDSourceID(final AsciiSequenceView view)
    {
        if (!hasAltMDSourceID)
        {
            throw new IllegalArgumentException("No value for optional field: AltMDSourceID");
        ...
        return view.wrap(buffer, altMDSourceIDOffset, altMDSourceIDLength);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.M_D_REQ_REJ_REASON:
                ...
                mDReqRejReasonLength = valueLength;
                break;
            ...
            case Constants.NO_ALT_M_D_SOURCE_GROUP_COUNTER:
                ...
                for (int i = 0; i < noAltMDSourceGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = altMDSourceGroupCurrent.decode(buffer, position, end - position);
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (altMDSourceGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (altMDSourceGroupCurrent != null) 
                            {
