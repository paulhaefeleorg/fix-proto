...
import uk.co.real_logic.artio.builder.BidRequestEncoder;
import static uk.co.real_logic.artio.builder.BidRequestEncoder.BidDescriptorsGroupEncoder;
import static uk.co.real_logic.artio.builder.BidRequestEncoder.BidComponentsGroupEncoder;
...
public class BidRequestDecoder extends CommonDecoderImpl implements MessageDecoder
{
    ...
    public boolean validate()
    {
        ...
        if (CODEC_VALIDATION_ENABLED && bidRequestTransTypeLength > 1)
        {
            invalidTagId = 374;
            rejectReason = 5;
            return false;
        ...
        {
        if (CODEC_VALIDATION_ENABLED && exchangeForPhysicalLength > 1)
        {
            invalidTagId = 411;
            rejectReason = 5;
            return false;
        ...
        if (CODEC_VALIDATION_ENABLED && bidTradeTypeLength > 1)
        {
            invalidTagId = 418;
            rejectReason = 5;
            return false;
        ...
        if (CODEC_VALIDATION_ENABLED && basisPxTypeLength > 1)
        {
            invalidTagId = 419;
            rejectReason = 5;
            return false;
    ...
    private int bidRequestTransTypeLength = 0;
    public int bidRequestTransTypeLength()    {
       return bidRequestTransTypeLength;
    }
    ...
    /* ListName = 392 */
    public AsciiSequenceView listName(final AsciiSequenceView view)
    {
        if (!hasListName)
        {
            throw new IllegalArgumentException("No value for optional field: ListName");
        ...
        return view.wrap(buffer, listNameOffset, listNameLength);
    }
...
public class BidDescriptorsGroupDecoder extends CommonDecoderImpl
{
 ...
 return BidDescriptorType.NULL_VAL;
    ...
    /* BidDescriptor = 400 */
    public AsciiSequenceView bidDescriptor(final AsciiSequenceView view)
    {
        if (!hasBidDescriptor)
        {
            throw new IllegalArgumentException("No value for optional field: BidDescriptor");
        ...
        return view.wrap(buffer, bidDescriptorOffset, bidDescriptorLength);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.LIQUIDITY_VALUE:
                ...
                liquidityValue = getFloat(buffer, liquidityValue, valueOffset, valueLength, 404, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;
            ...
            case Constants.LIQUIDITY_PCT_LOW:
                ...
                liquidityPctLow = getFloat(buffer, liquidityPctLow, valueOffset, valueLength, 402, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.LIQUIDITY_PCT_HIGH:
                hasLiquidityPctHigh = true;
                liquidityPctHigh = getFloat(buffer, liquidityPctHigh, valueOffset, valueLength, 403, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.E_F_P_TRACKING_ERROR:
                hasEFPTrackingError = true;
                eFPTrackingError = getFloat(buffer, eFPTrackingError, valueOffset, valueLength, 405, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.FAIR_VALUE:
                hasFairValue = true;
                fairValue = getFloat(buffer, fairValue, valueOffset, valueLength, 406, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.OUTSIDE_INDEX_PCT:
                hasOutsideIndexPct = true;
                outsideIndexPct = getFloat(buffer, outsideIndexPct, valueOffset, valueLength, 407, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.VALUE_OF_FUTURES:
                hasValueOfFutures = true;
                valueOfFutures = getFloat(buffer, valueOfFutures, valueOffset, valueLength, 408, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;
...
public class BidComponentsGroupDecoder extends CommonDecoderImpl
{
 ...
 return LiquidityIndType.NULL_VAL;
    ...
    private int exchangeForPhysicalLength = 0;
    public int exchangeForPhysicalLength()    {
       return exchangeForPhysicalLength;
    }
 ...
 return IncTaxInd.NULL_VAL;
    ...
    private int bidTradeTypeLength = 0;
    public int bidTradeTypeLength()    {
       return bidTradeTypeLength;
    }
    /* BidTradeType = 418 */
    ...
    private int basisPxTypeLength = 0;
    public int basisPxTypeLength()    {
       return basisPxTypeLength;
    }
    /* BasisPxType = 419 */
    ...
    /* StrikeTime = 443 */
    public AsciiSequenceView strikeTime(final AsciiSequenceView view)
    {
        if (!hasStrikeTime)
        {
            throw new IllegalArgumentException("No value for optional field: StrikeTime");
        ...
        return view.wrap(buffer, strikeTimeOffset, strikeTimeLength);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.BID_REQUEST_TRANS_TYPE:
                ...
                bidRequestTransTypeLength = valueLength;
                break;
            ...
            case Constants.SIDE_VALUE1:
                hasSideValue1 = true;
                sideValue1 = getFloat(buffer, sideValue1, valueOffset, valueLength, 396, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.SIDE_VALUE2:
                hasSideValue2 = true;
                sideValue2 = getFloat(buffer, sideValue2, valueOffset, valueLength, 397, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;
            ...
            case Constants.NO_BID_DESCRIPTORS_GROUP_COUNTER:
                ...
                for (int i = 0; i < noBidDescriptorsGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = bidDescriptorsGroupCurrent.decode(buffer, position, end - position);
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (bidDescriptorsGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (bidDescriptorsGroupCurrent != null) 
                            {
            ...
            case Constants.NO_BID_COMPONENTS_GROUP_COUNTER:
                ...
                for (int i = 0; i < noBidComponentsGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = bidComponentsGroupCurrent.decode(buffer, position, end - position);
                        if (positionIter == 0 && CODEC_VALIDATION_ENABLED)
                        {
                                invalidTagId = tag;
                                rejectReason = 16;
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (bidComponentsGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (bidComponentsGroupCurrent != null) 
                            {
            ...
            case Constants.WT_AVERAGE_LIQUIDITY:
                ...
                wtAverageLiquidity = getFloat(buffer, wtAverageLiquidity, valueOffset, valueLength, 410, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.EXCHANGE_FOR_PHYSICAL:
                hasExchangeForPhysical = true;
                exchangeForPhysical = buffer.getBoolean(valueOffset);
                exchangeForPhysicalLength = valueLength;
                break;

            case Constants.OUT_MAIN_CNTRY_U_INDEX:
                hasOutMainCntryUIndex = true;
                outMainCntryUIndex = getFloat(buffer, outMainCntryUIndex, valueOffset, valueLength, 412, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;

            case Constants.CROSS_PERCENT:
                hasCrossPercent = true;
                crossPercent = getFloat(buffer, crossPercent, valueOffset, valueLength, 413, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;
