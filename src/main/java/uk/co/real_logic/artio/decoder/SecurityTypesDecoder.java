...
import uk.co.real_logic.artio.builder.SecurityTypesEncoder;
import static uk.co.real_logic.artio.builder.SecurityTypesEncoder.SecurityTypesGroupEncoder;
        ...
        while (position < end)
        {
            ...
            case Constants.NO_SECURITY_TYPES_GROUP_COUNTER:
                ...
                for (int i = 0; i < noSecurityTypesGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = securityTypesGroupCurrent.decode(buffer, position, end - position);
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (securityTypesGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (securityTypesGroupCurrent != null) 
                            {
