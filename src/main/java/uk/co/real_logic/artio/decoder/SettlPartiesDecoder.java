...
public class SettlPartyIDsGroupDecoder extends CommonDecoderImpl
{
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_VALIDATION_ENABLED && settlPartyIDSourceLength > 1)
        {
            invalidTagId = 783;
            rejectReason = 5;
            return false;
    ...
    /* SettlPartyID = 782 */
    public AsciiSequenceView settlPartyID(final AsciiSequenceView view)
    {
        if (!hasSettlPartyID)
        {
            throw new IllegalArgumentException("No value for optional field: SettlPartyID");
        ...
        return view.wrap(buffer, settlPartyIDOffset, settlPartyIDLength);
    }
    ...
    private int settlPartyIDSourceLength = 0;
    public int settlPartyIDSourceLength()    {
       return settlPartyIDSourceLength;
    }
...
public class SettlPartySubIDsGroupDecoder extends CommonDecoderImpl
{
    ...
    /* SettlPartySubID = 785 */
    public AsciiSequenceView settlPartySubID(final AsciiSequenceView view)
    {
        if (!hasSettlPartySubID)
        {
            throw new IllegalArgumentException("No value for optional field: SettlPartySubID");
        ...
        return view.wrap(buffer, settlPartySubIDOffset, settlPartySubIDLength);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.SETTL_PARTY_ID_SOURCE:
                ...
                settlPartyIDSourceLength = valueLength;
                break;
            ...
            case Constants.NO_SETTL_PARTY_SUB_IDS_GROUP_COUNTER:
                ...
                for (int i = 0; i < noSettlPartySubIDsGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = settlPartySubIDsGroupCurrent.decode(buffer, position, end - position);
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (settlPartySubIDsGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (settlPartySubIDsGroupCurrent != null) 
                            {
