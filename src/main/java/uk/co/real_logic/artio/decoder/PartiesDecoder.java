...
public class PartyIDsGroupDecoder extends CommonDecoderImpl
{
    ...
    public boolean validate()
    {
        ...
        if (hasPartyIDSource)
        {
        if (CODEC_VALIDATION_ENABLED && partyIDSourceLength > 1)
        {
            invalidTagId = 447;
            rejectReason = 5;
            return false;
    ...
    /* PartyID = 448 */
    public AsciiSequenceView partyID(final AsciiSequenceView view)
    {
        if (!hasPartyID)
        {
            throw new IllegalArgumentException("No value for optional field: PartyID");
        ...
        return view.wrap(buffer, partyIDOffset, partyIDLength);
    }
    ...
    private int partyIDSourceLength = 0;
    public int partyIDSourceLength()    {
       return partyIDSourceLength;
    }
...
public class PartySubIDsGroupDecoder extends CommonDecoderImpl
{
    ...
    /* PartySubID = 523 */
    public AsciiSequenceView partySubID(final AsciiSequenceView view)
    {
        if (!hasPartySubID)
        {
            throw new IllegalArgumentException("No value for optional field: PartySubID");
        ...
        return view.wrap(buffer, partySubIDOffset, partySubIDLength);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.PARTY_ID_SOURCE:
                ...
                partyIDSourceLength = valueLength;
                break;
            ...
            case Constants.NO_PARTY_SUB_IDS_GROUP_COUNTER:
                ...
                for (int i = 0; i < noPartySubIDsGroupCounter && position < end; i++)
                {
                    ...
                    {
                        positionIter = partySubIDsGroupCurrent.decode(buffer, position, end - position);
                ...
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (partySubIDsGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            while (partySubIDsGroupCurrent != null) 
                            {
