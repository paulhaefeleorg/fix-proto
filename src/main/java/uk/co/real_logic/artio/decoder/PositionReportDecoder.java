...
import uk.co.real_logic.artio.builder.PositionReportEncoder;
...
import static uk.co.real_logic.artio.builder.PositionReportEncoder.LegsGroupEncoder;
...
import static uk.co.real_logic.artio.builder.PositionReportEncoder.UnderlyingsGroupEncoder;
...
public class PositionReportDecoder extends CommonDecoderImpl implements PartiesDecoder, InstrumentDecoder, PositionQtyDecoder, PositionAmountDataDecoder, MessageDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            REQUIRED_FIELDS.add(Constants.POS_REQ_RESULT);
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.TOTAL_NUM_POS_REPORTS);
            GROUP_FIELDS.add(Constants.UNSOLICITED_INDICATOR);
            GROUP_FIELDS.add(Constants.POS_REQ_RESULT);
            ...
            GROUP_FIELDS.add(Constants.DELIVERY_DATE);
    ...
    private final IntHashSet alreadyVisitedFields = new IntHashSet(126);
    ...
    public boolean validate()
    {
        ...
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !PosReqResult.isValid(posReqResult()))
        {
            invalidTagId = 728;
            rejectReason = 5;
            return false;
    ...
    public static final long MESSAGE_TYPE = 20545L;

    public static final String MESSAGE_TYPE_AS_STRING = "AP";

    public static final char[] MESSAGE_TYPE_CHARS = MESSAGE_TYPE_AS_STRING.toCharArray();

    public static final byte[] MESSAGE_TYPE_BYTES = MESSAGE_TYPE_AS_STRING.getBytes(US_ASCII);

    public final IntHashSet messageFields = new IntHashSet(442);

    {
        ...
        messageFields.add(Constants.TOTAL_NUM_POS_REPORTS);
        messageFields.add(Constants.UNSOLICITED_INDICATOR);
        messageFields.add(Constants.POS_REQ_RESULT);
        ...
        messageFields.add(Constants.UNDERLYING_SETTL_PRICE_TYPE);
        ...
        messageFields.add(Constants.DELIVERY_DATE);
 ...
 return SubscriptionRequestType.NULL_VAL;
    ...
    private int totalNumPosReports = MISSING_INT;
    ...
    public int totalNumPosReports()
    {
        ...
        {
            throw new IllegalArgumentException("No value for optional field: TotalNumPosReports");
        }
    ...
    private int posReqResult = MISSING_INT;
    ...
    private final CharArrayWrapper posReqResultWrapper = new CharArrayWrapper();
    /* PosReqResult = 728 */
    public PosReqResult posReqResultAsEnum()
    {
        return PosReqResult.decode(posReqResult);
    }
...
public class LegsGroupDecoder extends CommonDecoderImpl implements InstrumentLegDecoder
{
    ...
    public PositionReportEncoder.LegsGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((PositionReportEncoder.LegsGroupEncoder)encoder);
    }

    public PositionReportEncoder.LegsGroupEncoder toEncoder(final PositionReportEncoder.LegsGroupEncoder encoder)
    {
        encoder.reset();
    ...
    {
        private final PositionReportDecoder parent;
        private int remainder;
        private LegsGroupDecoder current;

        public LegsGroupIterator(final PositionReportDecoder parent)
        {
            this.parent = parent;
        }
...
public class UnderlyingsGroupDecoder extends CommonDecoderImpl implements UnderlyingInstrumentDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            REQUIRED_FIELDS.add(Constants.UNDERLYING_SETTL_PRICE_TYPE);
        }
    }

    public final IntHashSet GROUP_FIELDS = new IntHashSet(94);
 ...
 return UnderlyingPutOrCall.NULL_VAL;
    ...
    {
        underlyingSettlPriceType = MISSING_INT;
    }
    ...
    public PositionReportEncoder.UnderlyingsGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((PositionReportEncoder.UnderlyingsGroupEncoder)encoder);
    }

    public PositionReportEncoder.UnderlyingsGroupEncoder toEncoder(final PositionReportEncoder.UnderlyingsGroupEncoder encoder)
    {
        encoder.reset();
        ...
        encoder.underlyingSettlPriceType(this.underlyingSettlPriceType());        return encoder;
    }
    ...
    public class UnderlyingsGroupIterator implements Iterable<UnderlyingsGroupDecoder>, java.util.Iterator<UnderlyingsGroupDecoder>
    {
        ...
        public UnderlyingsGroupIterator(final PositionReportDecoder parent)
        {
            this.parent = parent;
        }
    ...
    public char registStatus()
    {
        if (!hasRegistStatus)
        {
            throw new IllegalArgumentException("No value for optional field: RegistStatus");
        }
    ...
    {
        if (!hasRegistStatus)
 return RegistStatus.NULL_VAL;
        return RegistStatus.decode(registStatus);
    }

    private byte[] deliveryDate = new byte[8];

    private boolean hasDeliveryDate;

    /* DeliveryDate = 743 */
    public byte[] deliveryDate()
    {
        if (!hasDeliveryDate)
        {
            throw new IllegalArgumentException("No value for optional field: DeliveryDate");
        }
    ...
    private int deliveryDateOffset;

    private int deliveryDateLength;

    /* DeliveryDate = 743 */
    public int deliveryDateLength()
    {
        if (!hasDeliveryDate)
        {
            throw new IllegalArgumentException("No value for optional field: DeliveryDate");
        ...
        return deliveryDateLength;
    }

    /* DeliveryDate = 743 */
    public String deliveryDateAsString()
    {
        return hasDeliveryDate ? new String(deliveryDate, 0, deliveryDateLength) : null;
    }

    /* DeliveryDate = 743 */
    public AsciiSequenceView deliveryDate(final AsciiSequenceView view)
    {
        if (!hasDeliveryDate)
        {
            throw new IllegalArgumentException("No value for optional field: DeliveryDate");
        ...
        return view.wrap(buffer, deliveryDateOffset, deliveryDateLength);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.TOTAL_NUM_POS_REPORTS:
                hasTotalNumPosReports = true;
                totalNumPosReports = getInt(buffer, valueOffset, endOfField, 727, CODEC_VALIDATION_ENABLED);
                break;
            ...
            case Constants.POS_REQ_RESULT:
                posReqResult = getInt(buffer, valueOffset, endOfField, 728, CODEC_VALIDATION_ENABLED);
                break;
            ...
            case Constants.PRIOR_SETTL_PRICE:
                priorSettlPrice = getFloat(buffer, priorSettlPrice, valueOffset, valueLength, 734, CODEC_VALIDATION_ENABLED, decimalFloatOverflowHandler);
                break;
            ...
            case Constants.DELIVERY_DATE:
                hasDeliveryDate = true;
                deliveryDate = buffer.getBytes(deliveryDate, valueOffset, valueLength);
                deliveryDateOffset = valueOffset;
                deliveryDateLength = valueLength;
                break;
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        if (hasDeliveryDate())
        {
            ...
            appendData(builder, deliveryDate, deliveryDateLength);
            builder.append("\",\n");
        }
    ...
    public PositionReportEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((PositionReportEncoder)encoder);
    }

    public PositionReportEncoder toEncoder(final PositionReportEncoder encoder)
    {
        encoder.reset();
        ...
        if (hasDeliveryDate())
        {
            encoder.deliveryDateAsCopy(this.deliveryDate(), 0, deliveryDateLength());
        }
