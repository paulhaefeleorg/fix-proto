...
public class RegistrationInstructionsEncoder implements Encoder
{
    ...
    private byte[] registRefIDInternalBuffer = registRefID.byteArray();
    ...
    {
        if (copyInto(registRefID, value, offset, length))
        {
            registRefIDInternalBuffer = registRefID.byteArray();
        }
        registRefIDOffset = 0;
        registRefIDLength = length;
        return this;
    ...
    {
        if (toBytes(value, registRefID, offset, length))
        {
            registRefIDInternalBuffer = registRefID.byteArray();
        }
        registRefIDOffset = 0;
    ...
    private byte[] registAcctTypeInternalBuffer = registAcctType.byteArray();
    ...
    {
        if (copyInto(registAcctType, value, offset, length))
        {
            registAcctTypeInternalBuffer = registAcctType.byteArray();
        }
    ...
    {
        if (toBytes(value, registAcctType))
        {
            registAcctTypeInternalBuffer = registAcctType.byteArray();
        }
        registAcctTypeOffset = 0;
    ...
    {
        if (toBytes(value, registAcctType, offset, length))
        {
            registAcctTypeInternalBuffer = registAcctType.byteArray();
        }
        registAcctTypeOffset = 0;
...
public static class RegistDtlsGroupEncoder
{
    ...
    private byte[] registDtlsInternalBuffer = registDtls.byteArray();
    ...
    {
        if (copyInto(registDtls, value, offset, length))
        {
            registDtlsInternalBuffer = registDtls.byteArray();
        }
        registDtlsOffset = 0;
        registDtlsLength = length;
        return this;
    ...
    {
        if (toBytes(value, registDtls, offset, length))
        {
            registDtlsInternalBuffer = registDtls.byteArray();
        }
        registDtlsOffset = 0;
    ...
    private byte[] registEmailInternalBuffer = registEmail.byteArray();
    ...
    {
        if (copyInto(registEmail, value, offset, length))
        {
            registEmailInternalBuffer = registEmail.byteArray();
        }
        registEmailOffset = 0;
        registEmailLength = length;
        return this;
    ...
    {
        if (toBytes(value, registEmail, offset, length))
        {
            registEmailInternalBuffer = registEmail.byteArray();
        }
        registEmailOffset = 0;
    ...
    private byte[] mailingDtlsInternalBuffer = mailingDtls.byteArray();
    ...
    {
        if (copyInto(mailingDtls, value, offset, length))
        {
            mailingDtlsInternalBuffer = mailingDtls.byteArray();
        }
        mailingDtlsOffset = 0;
        mailingDtlsLength = length;
        return this;
    ...
    {
        if (toBytes(value, mailingDtls, offset, length))
        {
            mailingDtlsInternalBuffer = mailingDtls.byteArray();
        }
        mailingDtlsOffset = 0;
    ...
    private byte[] mailingInstInternalBuffer = mailingInst.byteArray();
    ...
    {
        if (copyInto(mailingInst, value, offset, length))
        {
            mailingInstInternalBuffer = mailingInst.byteArray();
        }
        mailingInstOffset = 0;
        mailingInstLength = length;
        return this;
    ...
    {
        if (toBytes(value, mailingInst, offset, length))
        {
            mailingInstInternalBuffer = mailingInst.byteArray();
        }
        mailingInstOffset = 0;
    ...
    private byte[] dateOfBirthInternalBuffer = dateOfBirth.byteArray();
    ...
    {
        if (copyInto(dateOfBirth, value, offset, length))
        {
            dateOfBirthInternalBuffer = dateOfBirth.byteArray();
        }
        dateOfBirthOffset = 0;
        dateOfBirthLength = length;
        return this;
    ...
    private byte[] investorCountryOfResidenceInternalBuffer = investorCountryOfResidence.byteArray();
    ...
    {
        if (copyInto(investorCountryOfResidence, value, offset, length))
        {
            investorCountryOfResidenceInternalBuffer = investorCountryOfResidence.byteArray();
        }
    ...
    {
        if (toBytes(value, investorCountryOfResidence))
        {
            investorCountryOfResidenceInternalBuffer = investorCountryOfResidence.byteArray();
        }
        investorCountryOfResidenceOffset = 0;
    ...
    {
        if (toBytes(value, investorCountryOfResidence, offset, length))
        {
            investorCountryOfResidenceInternalBuffer = investorCountryOfResidence.byteArray();
        }
        investorCountryOfResidenceOffset = 0;
    ...
    {
        registDtlsLength = 0;
        registDtls.wrap(registDtlsInternalBuffer);
    }

    public void resetRegistEmail()
    {
        registEmailLength = 0;
        registEmail.wrap(registEmailInternalBuffer);
    }

    public void resetMailingDtls()
    {
        mailingDtlsLength = 0;
        mailingDtls.wrap(mailingDtlsInternalBuffer);
    }

    public void resetMailingInst()
    {
        mailingInstLength = 0;
        mailingInst.wrap(mailingInstInternalBuffer);
    }
    ...
    {
        dateOfBirthLength = 0;
        dateOfBirth.wrap(dateOfBirthInternalBuffer);
    }

    public void resetInvestorCountryOfResidence()
    {
        investorCountryOfResidenceLength = 0;
        investorCountryOfResidence.wrap(investorCountryOfResidenceInternalBuffer);
    }
...
public static class DistribInstsGroupEncoder
{
    ...
    public DistribInstsGroupEncoder distribPercentage(ReadOnlyDecimalFloat value)
    {
    ...
    private byte[] cashDistribCurrInternalBuffer = cashDistribCurr.byteArray();
    ...
    {
        if (copyInto(cashDistribCurr, value, offset, length))
        {
            cashDistribCurrInternalBuffer = cashDistribCurr.byteArray();
        }
    ...
    {
        if (toBytes(value, cashDistribCurr))
        {
            cashDistribCurrInternalBuffer = cashDistribCurr.byteArray();
        }
        cashDistribCurrOffset = 0;
    ...
    {
        if (toBytes(value, cashDistribCurr, offset, length))
        {
            cashDistribCurrInternalBuffer = cashDistribCurr.byteArray();
        }
        cashDistribCurrOffset = 0;
    ...
    private byte[] cashDistribAgentNameInternalBuffer = cashDistribAgentName.byteArray();
    ...
    {
        if (copyInto(cashDistribAgentName, value, offset, length))
        {
            cashDistribAgentNameInternalBuffer = cashDistribAgentName.byteArray();
        }
    ...
    {
        if (toBytes(value, cashDistribAgentName))
        {
            cashDistribAgentNameInternalBuffer = cashDistribAgentName.byteArray();
        }
        cashDistribAgentNameOffset = 0;
    ...
    {
        if (toBytes(value, cashDistribAgentName, offset, length))
        {
            cashDistribAgentNameInternalBuffer = cashDistribAgentName.byteArray();
        }
        cashDistribAgentNameOffset = 0;
    ...
    private byte[] cashDistribAgentCodeInternalBuffer = cashDistribAgentCode.byteArray();
    ...
    {
        if (copyInto(cashDistribAgentCode, value, offset, length))
        {
            cashDistribAgentCodeInternalBuffer = cashDistribAgentCode.byteArray();
        }
    ...
    {
        if (toBytes(value, cashDistribAgentCode))
        {
            cashDistribAgentCodeInternalBuffer = cashDistribAgentCode.byteArray();
        }
        cashDistribAgentCodeOffset = 0;
    ...
    {
        if (toBytes(value, cashDistribAgentCode, offset, length))
        {
            cashDistribAgentCodeInternalBuffer = cashDistribAgentCode.byteArray();
        }
        cashDistribAgentCodeOffset = 0;
    ...
    private byte[] cashDistribAgentAcctNumberInternalBuffer = cashDistribAgentAcctNumber.byteArray();
    ...
    {
        if (copyInto(cashDistribAgentAcctNumber, value, offset, length))
        {
            cashDistribAgentAcctNumberInternalBuffer = cashDistribAgentAcctNumber.byteArray();
        }
    ...
    {
        if (toBytes(value, cashDistribAgentAcctNumber))
        {
            cashDistribAgentAcctNumberInternalBuffer = cashDistribAgentAcctNumber.byteArray();
        }
        cashDistribAgentAcctNumberOffset = 0;
    ...
    {
        if (toBytes(value, cashDistribAgentAcctNumber, offset, length))
        {
            cashDistribAgentAcctNumberInternalBuffer = cashDistribAgentAcctNumber.byteArray();
        }
        cashDistribAgentAcctNumberOffset = 0;
    ...
    private byte[] cashDistribPayRefInternalBuffer = cashDistribPayRef.byteArray();
    ...
    {
        if (copyInto(cashDistribPayRef, value, offset, length))
        {
            cashDistribPayRefInternalBuffer = cashDistribPayRef.byteArray();
        }
    ...
    {
        if (toBytes(value, cashDistribPayRef))
        {
            cashDistribPayRefInternalBuffer = cashDistribPayRef.byteArray();
        }
        cashDistribPayRefOffset = 0;
    ...
    {
        if (toBytes(value, cashDistribPayRef, offset, length))
        {
            cashDistribPayRefInternalBuffer = cashDistribPayRef.byteArray();
        }
        cashDistribPayRefOffset = 0;
    ...
    private byte[] cashDistribAgentAcctNameInternalBuffer = cashDistribAgentAcctName.byteArray();
    ...
    {
        if (copyInto(cashDistribAgentAcctName, value, offset, length))
        {
            cashDistribAgentAcctNameInternalBuffer = cashDistribAgentAcctName.byteArray();
        }
    ...
    {
        if (toBytes(value, cashDistribAgentAcctName))
        {
            cashDistribAgentAcctNameInternalBuffer = cashDistribAgentAcctName.byteArray();
        }
        cashDistribAgentAcctNameOffset = 0;
    ...
    {
        if (toBytes(value, cashDistribAgentAcctName, offset, length))
        {
            cashDistribAgentAcctNameInternalBuffer = cashDistribAgentAcctName.byteArray();
        }
        cashDistribAgentAcctNameOffset = 0;
    ...
    public void resetCashDistribCurr()
    {
        ...
        cashDistribCurr.wrap(cashDistribCurrInternalBuffer);
    }

    public void resetCashDistribAgentName()
    {
        cashDistribAgentNameLength = 0;
        cashDistribAgentName.wrap(cashDistribAgentNameInternalBuffer);
    }

    public void resetCashDistribAgentCode()
    {
        cashDistribAgentCodeLength = 0;
        cashDistribAgentCode.wrap(cashDistribAgentCodeInternalBuffer);
    }

    public void resetCashDistribAgentAcctNumber()
    {
        cashDistribAgentAcctNumberLength = 0;
        cashDistribAgentAcctNumber.wrap(cashDistribAgentAcctNumberInternalBuffer);
    }

    public void resetCashDistribPayRef()
    {
        cashDistribPayRefLength = 0;
        cashDistribPayRef.wrap(cashDistribPayRefInternalBuffer);
    }

    public void resetCashDistribAgentAcctName()
    {
        cashDistribAgentAcctNameLength = 0;
        cashDistribAgentAcctName.wrap(cashDistribAgentAcctNameInternalBuffer);
    }
    ...
    {
        registRefIDLength = 0;
        registRefID.wrap(registRefIDInternalBuffer);
    }
    ...
    public void resetRegistAcctType()
    {
        ...
        registAcctType.wrap(registAcctTypeInternalBuffer);
    }
